...
// **************************************************************************
// int test7( )
//   Performs some basic tests of begin and end.
//   Returns POINTS[7] if the tests are passed. Otherwise returns 0.
// **************************************************************************
int test7( )
{
    const size_t TESTSIZE = 15;
    sequence<double> testa;
    double items[2*TESTSIZE];
    size_t i;
    ...
    cout << "Attaching sequence contents and checking validity" << endl;
    for (i = 1; i <= 2*TESTSIZE; i++)
        testa.attach(i);
    if (!correct(testa, 2*TESTSIZE, 2*TESTSIZE-1, items))
        return 0;
    
    cout << "Testing to see that begin and end can be properly used\n";
    cout << "to iterate through the sequence. Sequence contents:" << endl;
    i = 1;
    for (sequence<double>::iterator itr = testa.begin(); itr != testa.end(); ++itr, ++i)
    {
        // Check for null iterator
        if (itr == node_iterator<double>())
        {
            cout << "\nUnexpected NULL iterator found." << endl;
            return 0;
        }
...
int main( int argc, char *argv[])
{
    ...
    if (testingMethod == 1)
    {
        ...
        cout << "If you submit this sequence to now, you will have\n";
